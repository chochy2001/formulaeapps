...
/// introduces zero behavior change until Formulae Pro has its own Firebase
/// project + PostHog credentials wired in CI. No key is hardcoded here.
///
...
///
/// Pattern mirrors CapGym#79 (`observability_bootstrap.dart`).
...
/// PostHog fleet-lock super property for Formulae Pro. Community builds pass
/// [postHogAppPropertyCommunity] via [bootstrapPostHog]'s `appProperty`.
const String postHogAppPropertyPro = 'formulae-pro';

const String postHogAppPropertyCommunity = 'formulae-community';
...
/// set.
Future<void> bootstrapObservability({
  String appProperty = postHogAppPropertyPro,
}) async {
  await bootstrapCrashlytics();
  await bootstrapPostHog(appProperty: appProperty);
}
...
Future<void> bootstrapPostHog({
  ...
  try {
    ...
    await Posthog().register('app', appProperty);
  } catch (e) {
