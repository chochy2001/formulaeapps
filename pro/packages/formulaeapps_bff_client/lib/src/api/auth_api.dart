...
class AuthApi {
  ...
  Future<Response<AccountAuthResponse>> authRegisterPost({ 
    ...
    try {
      const _type = FullType(AccountRegisterRequest);
      _bodyData = _serializers.serialize(accountRegisterRequest, specifiedType: _type);
  ...
  /// Issue a session JWT
  /// Verifies the client_proof HMAC and issues a short-lived (≤60min) HS256 JWT. Called by the FE at app cold start and again when the previous token expires.
  ///
  /// Parameters:
  /// * [authTokenRequest] 
  /// * [cancelToken] - A [CancelToken] that can be used to cancel the operation
  ...
  /// * [onReceiveProgress] - A [ProgressCallback] that can be used to get the receive progress
  ///
  /// Returns a [Future] containing a [Response] with a [AuthTokenResponse] as data
  /// Throws [DioException] if API call or serialization fails
  Future<Response<AuthTokenResponse>> authTokenPost({ 
    ...
    try {
      const _type = FullType(AuthTokenRequest);
      _bodyData = _serializers.serialize(authTokenRequest, specifiedType: _type);
    ...
    AuthTokenResponse? _responseData;

    try {
      final rawResponse = _response.data;
      _responseData = rawResponse == null ? null : _serializers.deserialize(
        rawResponse,
        specifiedType: const FullType(AuthTokenResponse),
