// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'serializers.dart';

// **************************************************************************
// BuiltValueGenerator
// **************************************************************************

Serializers _$serializers =
    (Serializers().toBuilder()
          ..add(AccountAuthResponse.serializer)
          ..add(AccountLoginRequest.serializer)
          ..add(AccountLoginRequestPlatformEnum.serializer)
          ..add(AccountOAuthRequest.serializer)
          ..add(AccountOAuthRequestPlatformEnum.serializer)
          ..add(AccountOAuthRequestProviderEnum.serializer)
          ..add(AccountRegisterRequest.serializer)
          ..add(AccountRegisterRequestPlatformEnum.serializer)
          ..add(AuthTokenRequest.serializer)
          ..add(AuthTokenRequestPlatformEnum.serializer)
          ..add(AuthTokenResponse.serializer)
          ..add(ChatRequest.serializer)
          ..add(ChatResponse.serializer)
          ..add(ChatUsage.serializer)
          ..add(EntitlementResponse.serializer)
          ..add(EntitlementResponseScopeEnum.serializer)
          ..add(EntitlementSource.serializer)
          ..add(EntitlementSourcePaymentSourceEnum.serializer)
          ..add(ErrorEnvelope.serializer)
          ..add(ErrorEnvelopeError.serializer)
          ..add(ErrorKind.serializer)
          ..add(HealthResponse.serializer)
          ..add(HealthStatus.serializer)
          ..add(IapValidateRequest.serializer)
          ..add(IapValidateRequestPlatformEnum.serializer)
          ..add(IapValidateResponse.serializer)
          ..add(IapValidateResponseEnvironmentEnum.serializer)
          ..addBuilderFactory(
            const FullType(BuiltList, const [
              const FullType(EntitlementSource),
            ]),
            () => ListBuilder<EntitlementSource>(),
          ))
        .build();

// ignore_for_file: deprecated_member_use_from_same_package,type=lint
