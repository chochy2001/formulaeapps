...
la rama `agent/formulae-image-regeneration-20260713`, con base local
`be796b1`. Sustituye las afirmaciones no fechadas de "producción lista" en la
documentación histórica. No autoriza despliegues, publicaciones en tiendas ni
...
| Landing | El validador local de assets pasa. La landing ya tiene redirecciones de compatibilidad en `.htaccess` y `nginx.conf`. |
...
| BFF | `bun run typecheck` y `bun test` pasaron con 125 pruebas en 20 archivos. El scanner de rutas ya reconoce la llamada real generada `iapValidatePost` en lugar de contar un comentario como consumidor. |
...
xcrun simctl launch --console-pty booted capdesis.formulae
...
La landing pasó lint, 64 pruebas, build de 13 páginas y validador de 176
assets locales. Pro pasó análisis estricto, 77 pruebas y build web; Tareas y
Favoritos se revisaron en navegador local a 320 y 900 px sin overflow ni
errores de consola. Community pasó 95 pruebas, incluidas las regresiones
...
1. `bun run check:formulae-images:remote` falla 176 de 176. Las URLs
   `https://formulaeapps.com/imagenes/...` responden 404 aunque el set local
   está completo. Hay que promover exactamente `landing/public/imagenes/` y las
   reglas de compatibilidad al hosting autorizado, después correr el smoke
   remoto otra vez.
...
3. `bash scripts/infra-validate.sh --local` no puede completar CORS ni BFF
   porque falta el `.env` raíz y no existe un BFF local corriendo. Crear o usar
   ese entorno es una operación del operador; los secretos no se agregan al repo.
4. Tras `git fetch origin` durante la revisión, `HEAD=be796b1` y
   `origin/main=d475fc8` (`0 11` en `HEAD...origin/main`). No se hizo commit,
   merge ni rebase porque contiene cambios locales pendientes y no existe
   autorización para esa integración. Revalidar sobre el SHA de promoción antes
   de liberar.
...
4. Proveer el `.env` local fuera de Git, levantar el BFF y repetir la validación
   de infraestructura y CORS.
...
6. Ejecutar los quality gates del SHA exacto que se vaya a promover. No basar
