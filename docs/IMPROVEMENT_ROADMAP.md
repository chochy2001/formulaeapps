...
2. **Direct prod BFF cutover to contract 2.1.0 (#9 / #13 / FML-116)** — enable entitlement/accounts behind flags with evidence.
...
7. **Accounts UI / device pairing / Polar** — only after direct-production cutover + product decisions (T42); keep flags off.
...
| **2** | Direct-production BFF deploy with exact-main, backup, smoke, and rollback evidence |
...
FML-116 runners/production controls ──► FML-127 exact SHA ──► #9/#13 VPS cutover ──► BFF 2.1.0
FML-129 rotate key ──► safe chat release
FML-117 (+ T40 secrets) ──► IAP/accounts go-live decisions
T41 / T42 ──► VPS backups, Polar, Community drift, STORE_AUTODEPLOY, historic PDFs
```
...
1. Implement features only behind existing flags until direct-production cutover evidence exists.
